    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < sensor_fusion.size(); ++i){
            ...
            if ( ( lane_id*lane_width < sensor_fusion[i][6]) &&  
              ...
              if ((sensor_fusion[i][5] < (end_path_s + safety_dist)) && 
                ...
                // calculate absolute speed value in m/s of the other car          
                ...
                // if car slower than eog car, adjust to its speed
                if (delta_speed > 0.0){
            ...
            // check for cars on my right in the eventuality of a lane change to the right
            // make sure ego car is not in the foremost right lane           
            if ( (lane_id < 2) &&
                 ((lane_id+1) * lane_width < sensor_fusion[i][6]) &&  
                 (sensor_fusion[i][6] < (lane_id+2) * lane_width) ){
              // look for the space that is available in the right lane
              if ( (sensor_fusion[i][5] < car_s + 2*safety_dist) && 
                   (sensor_fusion[i][5] > car_s - safety_dist/2)   ){
                right_clear = 0;
              }
          ...
          // if the car in front of is much too slow, change lane if not already changing lane
          ...
          // get end points from previous path and angle
          // if path empty or too short, get the current position and angle of the car
          if (path_size == 0 || path_size == 1) {
            ...
            // add last two points to spline list for better trajectory transition
          ...
          // calculate spline points ahead of the ego vehicle, spaced by spline_dist
